...
# Base R way
ll_sample <- linelist[sample(nrow(linelist), 100),]
datatable(ll_sample)
...
```{r}
infdeath <- read.table("../data/infdeath.txt", header = TRUE, sep = "\t")
```

## Example Analysis - Infant Mortality 

```{r}
plot(infdeath$breast, infdeath$death, xlab="% Breast Feeding", ylab="Infant Death Rate")
```
```{r}
ggplot(infdeath, aes(x = safe, y = breast)) + 
  geom_point() +
  xlab("% Access Safe Water") + 
  ylab("% Breast Feeding")
```
...
plot(lm1$model$breast, lm1$residuals, xlab="Breastfeeding", ylab = "Residuals")
abline(h = 0)
```
...
```{r}
logit1 <- svyglm(dental.caries ~ diabetes + DMDBORN.cat + BMXBMI.cat, family = quasibinomial, design = ado_design, na.action = na.omit )
summary(logit1)
```
