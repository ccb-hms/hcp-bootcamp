...
Select gets certain columns of a tibble.

```{r}
# Base R way
nhanes$RIDAGEYR 

nhanes[,c("RIDAGEYR", "DMDBORN")] #selecting columns

nhanes[1:100,1:5] #selecting rows and columns

nhanes[nhanes$LBXGH > 5.7,]

#tidyverse way
select(nhanes, RIDAGEYR) 
select(nhanes, -RIDAGEYR, -DMDBORN, -SEQN)

filter(nhanes, RIAGENDR == "Male" & LBXGLU > 80 & `...1` < 100)
slice(nhanes, 1:100)

# Missing data with filter
sum(is.na(nhanes$LBXGH))
filter(nhanes, LBXGH > 5.7) |> select(LBXGH)
...
# Alternative 1
nhanes2 <- filter(nhanes, LBXGH > 5.7)
select(nhanes2, LBXGH)

# Alternative 2
select(filter(nhanes, LBXGH > 5.7), LBXGH)

```

In piping `f(x) |> g()` becomes `g(f(x))`.
...
[R Epi Chapter on tables](https://epirhandbook.com/en/descriptive-tables.html)

### Table

Table counts data and gives you a count table. 
...
nhanes |>
  select(dental.caries, BMXBMI.cat, LBXGH.cat, DMDBORN, diabetes) |>
  tbl_summary(by = dental.caries) |>
  modify_header(label = "**Dental Caries**") |>
  add_n() |>
  add_p()
```
...
Side note: Named colors in R [R graph gallery](https://r-graph-gallery.com/42-colors-names.html)

```{r}
# library(ggplot2) we shouldn't need this since it's in tidyverse

myplot <- ggplot(nhanes, aes(x = LBXGH, y= LBXGLU))

myplot + geom_point(color = "blue")
```
```{r}
myplot + geom_hex(bins = 50)
```
Boxplot:
...
ggplot(nhanes, aes(x = RIDRETH1, y = BMXBMI)) + 
  geom_violin()
```
## Mapping Additional Variables and Customizing Plots

```{r}
ggplot(nhanes, aes(x = LBXGH, y = BMXBMI, color = diabetes)) +
  geom_point(alpha = 0.4, aes(shape = diabetes))
```
```{r}
ggplot(nhanes, aes(x = LBXGH, y = BMXBMI, color = diabetes)) +
...
ggplot(nhanes, aes(x = RIDRETH1, y = BMXBMI, fill = dental.caries)) + 
  geom_boxplot() +
  theme_minimal() + 
  ggtitle("BMI by ethnicity and dental caries") + 
  xlab("Ethnicity") + 
  ylab("BMI") + 
